...
class EACH:
    def __init__(
        ...
        self.__EP_LIN_SOLVER = _validate_ep_lin_solver(
            EP_LIN_SOLVER, errorLog=self.__errorLog
        )
        self.__GEO_OPT = _validate_geo_opt(GEO_OPT, errorLog=self.__errorLog)
        self.__JUST_ENERGY = _validate_just_energy(
            JUST_ENERGY, errorLog=self.__errorLog
        )
        self.__MD = _validate_md(MD, errorLog=self.__errorLog)
        self.__METADYNAMICS = _validate_metadynamics(
            METADYNAMICS, errorLog=self.__errorLog
        )
        ...
        self.__REPLICA_EVAL = _validate_replica_eval(
            REPLICA_EVAL, errorLog=self.__errorLog
        )
        ...
        self.__SPLINE_FIND_COEFFS = _validate_spline_find_coeffs(
            SPLINE_FIND_COEFFS, errorLog=self.__errorLog
        )
    ...
    def SPLINE_FIND_COEFFS(self, val):
        if utilities.is_positive_integer(val):
            self.__changeLog.append(
                {
                    ...
                    "Variable": "SPLINE_FIND_COEFFS",
                    "Success": True,
                    "Previous": self.__SPLINE_FIND_COEFFS,
                    "New": val,
                    "ErrorMessage": None,
                    "Location": self.__location,
                }
            ...
            errorMessage = (
                "SPLINE_FIND_COEFFS iteration level must be a positive integer."
            )
            self.__changeLog.append(
                {
                    "Date": datetime.datetime.now(),
