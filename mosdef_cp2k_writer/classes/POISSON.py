...
from mosdef_cp2k_writer.classes import EWALD
...
def _validate_POISSON_SOLVER(val, errorLog=[]):
    ...
    if val in POISSON_SOLVER_VALS or (val is None):
        ...
        errorMessage = (
            "Invalid option for POISSON_SOLVER: {}. Valid options are: {}".format(
...
class POISSON:
    def __init__(
        self, PERIODIC=None, POISSON_SOLVER=None, errorLog=[], changeLog=[], location=""
    ):
        self.__errorLog = errorLog
        self.__changeLog = changeLog
        self.__location = "{}/POISSON".format(location)
        self.__PERIODIC = _validate_PERIODIC(PERIODIC, errorLog=self.__errorLog)
        self.__POISSON_SOLVER = _validate_POISSON_SOLVER(
            POISSON_SOLVER, errorLog=self.__errorLog
        )
        self.__EWALD = EWALD.EWALD(
            ...
            self.__changeLog.append(
                {
                    ...
                    "Variable": "POISSON_SOLVER",
                    "Success": True,
                    "Previous": self.__POISSON_SOLVER,
                    "New": val,
                    "ErrorMessage": None,
                    "Location": self.__location,
                }
