...
class SIM:
    def __init__(self):
        ...
        self.__scratchDirectory = "/tmp/cssi-cp2k-{}".format(
            int(random.random() * 123456789)
        )
        ...
        self.__GLOBAL = GLOBAL.GLOBAL(
            errorLog=self.__errorLog,
            changeLog=self.__changeLog,
            location=self.__location,
        )
        self.__MOTION = MOTION.MOTION(
            errorLog=self.__errorLog,
            changeLog=self.__changeLog,
            location=self.__location,
        )
        self.__FORCE_EVAL = FORCE_EVAL.FORCE_EVAL(
            errorLog=self.__errorLog,
            changeLog=self.__changeLog,
            location=self.__location,
        )
    ...
    def write_changeLog(self, success=None, keys=None, fn=None):
        ...
        for change in self.__changeLog:
            if keys is None:
                if success is None:
                    logString += "{}/{}\n\n".format(
                        change["Location"], change["Variable"]
                    )
                    logString += "{}: {}\n".format("New", change["New"])
                    logString += "{}: {}\n".format("Previous", change["Previous"])
                    logString += "{}: {}\n".format(
                        "Date", utilities.datetimePrettify(change["Date"])
                    )
