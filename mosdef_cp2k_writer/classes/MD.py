...
from mosdef_cp2k_writer.classes import BAROSTAT as barostat
...
def _validate_ensemble(val, errorLog=[]):
    ...
    if val in MD_ENSEMBLE_VALS or (val is None):
        ...
        errorMessage = (
            "Invalid option for MD ENSEMBLE: {}. Valid options are: {}".format(
                val, MD_ENSEMBLE_VALS
            )
        )
        errorLog.append(
            {
...
class MD:
    def __init__(
        ...
        self.__PRINT = md_print.PRINT(
            errorLog=self.__errorLog,
            changeLog=self.__changeLog,
            location=self.__location,
        )
        self.__AVERAGES = averages.AVERAGES(
            errorLog=self.__errorLog,
            changeLog=self.__changeLog,
            location=self.__location,
        )
    ...
    def TEMPERATURE(self, val):
        if utilities.is_positive_number(val) or val is None:
            ...
            errorMessage = (
                "TEMPERATURE must be a positive number. You provided {}".format(val)
            )
            self.__changeLog.append(
                {
                    "Date": datetime.datetime.now(),
