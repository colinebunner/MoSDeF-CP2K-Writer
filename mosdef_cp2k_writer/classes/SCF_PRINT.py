...
from mosdef_cp2k_writer.classes import SCF_RESTART
...
def _validate_DM_RESTART_WRITE(val, errorLog=[]):
    ...
    if val in BOOL_VALS or (val is None):
        ...
        errorMessage = (
            "Invalid option for DM_RESTART_WRITE: {}. Valid options are: {}".format(
                val, BOOL_VALS
            )
        )
        errorLog.append(
...
class PRINT:
    def __init__(self, DM_RESTART_WRITE=None, errorLog=[], changeLog=[], location=""):
        ...
        self.__DM_RESTART_WRITE = _validate_DM_RESTART_WRITE(
            DM_RESTART_WRITE, errorLog=self.__errorLog
        )
        self.__location = "{}/SCF_PRINT".format(location)
        self.__RESTART = SCF_RESTART.RESTART(
            ...
            self.__changeLog.append(
                {
                    ...
                    "Variable": "DM_RESTART_WRITE",
                    "Success": True,
                    "Previous": self.__DM_RESTART_WRITE,
                    "New": val,
                    "ErrorMessage": None,
                    "Location": self.__location,
                }
