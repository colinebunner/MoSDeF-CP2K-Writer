...
def _validate_COMPONENTS_TO_FIX(val, errorLog=[]):
    ...
    if val in COMPONENTS_VALS or (val is None):
        ...
        errorMessage = "Invalid option for COMPONENTS_VALS FIXED_ATOMS: {}. Valid options are: {}".format(
            val, COMPONENTS_VALS
        )
        errorLog.append(
            {
...
class FIXED_ATOMS:
    def __init__(
        self, COMPONENTS_TO_FIX=None, LIST=None, errorLog=[], changeLog=[], location=""
    ):

        self.__errorLog = errorLog
        self.__changeLog = changeLog
        self.__COMPONENTS_TO_FIX = _validate_COMPONENTS_TO_FIX(
            COMPONENTS_TO_FIX, errorLog=self.__errorLog
        )
    ...
    def COMPONENTS_TO_FIX(self, val):
        ...
        if val in COMPONENTS_VALS:
            self.__changeLog.append(
                {
                    "Date": datetime.datetime.now(),
                    "Module": "GEO_OPT",
                    "Variable": "COMPONENTS_TO_FIX",
                    "Success": True,
                    "Previous": self.__COMPONENTS_TO_FIX,
                    "New": val,
                    "ErrorMessage": None,
                    "Location": self.__location,
                }
            ...
            errorMessage = "Invalid option for COMPONENTS_TO_FIX in GEO_OPT: {}. Valid options are: {}".format(
                COMPONENTS_VALS
            )
            self.__changeLog.append(
                {
