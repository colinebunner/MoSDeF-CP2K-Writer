...
from mosdef_cp2k_writer.classes import OUTER_SCF
...
def _validate_NOTCONV_STOPALL(val, errorLog=[]):
    ...
    if val in BOOL_VALS or (val is None):
        ...
        errorMessage = (
            "Invalid option for NOTCONV_STOPALL: {}. Valid options are: {}".format(
                val, BOOL_VALS
            )
        )
        errorLog.append(
...
def _validate_ROKS_SCHEME(val, errorLog=[]):
    ...
    if val in ROKS_SCHEME_VALS or (val is None):
        ...
        errorMessage = (
            "Invalid option for ROKS_SCHEME: {}. Valid options are: {}".format(
...
class SCF:
    def __init__(
        ...
        self.__EPS_SCF_HISTORY = _validate_EPS_SCF_HISTORY(
            EPS_SCF_HISTORY, errorLog=self.__errorLog
        )
        self.__LEVEL_SHIFT = _validate_LEVEL_SHIFT(
            LEVEL_SHIFT, errorLog=self.__errorLog
        )
        self.__MAX_DIIS = _validate_MAX_DIIS(MAX_DIIS, errorLog=self.__errorLog)
        self.__MAX_ITER_LUMO = _validate_MAX_ITER_LUMO(
            MAX_ITER_LUMO, errorLog=self.__errorLog
        )
        self.__MAX_SCF = _validate_MAX_SCF(MAX_SCF, errorLog=self.__errorLog)
        self.__MAX_SCF_HISTORY = _validate_MAX_SCF_HISTORY(
            MAX_SCF_HISTORY, errorLog=self.__errorLog
        )
        self.__NCOL_BLOCK = _validate_NCOL_BLOCK(NCOL_BLOCK, errorLog=self.__errorLog)
        self.__NOTCONV_STOPALL = _validate_NOTCONV_STOPALL(
            NOTCONV_STOPALL, errorLog=self.__errorLog
        )
        ...
        self.__ROKS_PARAMETERS = _validate_ROKS_PARAMETERS(
            ROKS_PARAMETERS, errorLog=self.__errorLog
        )
        self.__ROKS_SCHEME = _validate_ROKS_SCHEME(
            ROKS_SCHEME, errorLog=self.__errorLog
        )
        ...
        self.__PRINT = SCF_PRINT.PRINT(
            errorLog=self.__errorLog,
            changeLog=self.__changeLog,
            location=self.__location,
        )
        self.__OUTER_SCF = OUTER_SCF.OUTER_SCF(
            errorLog=self.__errorLog,
            changeLog=self.__changeLog,
            location=self.__location,
        )
            ...
            self.__changeLog.append(
                {
                    ...
                    "Variable": "MAX_SCF_HISTORY",
                    "Success": True,
                    "Previous": self.__MAX_SCF_HISTORY,
                    "New": val,
                    "ErrorMessage": None,
                    "Location": self.__location,
                }
            ...
            self.__changeLog.append(
                {
                    ...
                    "Variable": "NOTCONV_STOPALL",
                    "Success": True,
                    "Previous": self.__NOTCONV_STOPALL,
                    "New": val,
                    "ErrorMessage": None,
                    "Location": self.__location,
                }
    ...
    def SCF_GUESS(self, val):
        ...
        if val in SCF_GUESS_VALS:
            ...
            errorMessage = (
                "Invalid option for SCF_GUESS: {}. Valid options are: {}".format(
                    val, SCF_GUESS_VALS
                )
            )
            self.__changeLog.append(
