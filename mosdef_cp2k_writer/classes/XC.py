...
from mosdef_cp2k_writer.classes import XC_FUNCTIONAL

from mosdef_cp2k_writer.classes import VDW_POTENTIAL
from mosdef_cp2k_writer.classes import XC_GRID
...
def _validate_FUNCTIONAL_ROUTINE(val, errorLog=[]):
    ...
    if val in FUNCTIONAL_ROUTINE_VALS or (val is None):
        ...
        errorMessage = (
            "Invalid option for FUNCTIONAL_ROUTINE: {}. Valid options are: {}".format(
...
class XC:
    def __init__(
        ...
        self.__DENSITY_CUTOFF = _validate_DENSITY_CUTOFF(
            DENSITY_CUTOFF, errorLog=self.__errorLog
        )
        self.__DENSITY_SMOOTH_CUTOFF_RANGE = _validate_DENSITY_SMOOTH_CUTOFF_RANGE(
            DENSITY_SMOOTH_CUTOFF_RANGE, errorLog=self.__errorLog
        )
        self.__FUNCTIONAL_ROUTINE = _validate_FUNCTIONAL_ROUTINE(
            FUNCTIONAL_ROUTINE, errorLog=self.__errorLog
        )
        self.__GRADIENT_CUTOFF = _validate_GRADIENT_CUTOFF(
            GRADIENT_CUTOFF, errorLog=self.__errorLog
        )
        self.__TAU_CUTOFF = _validate_TAU_CUTOFF(TAU_CUTOFF, errorLog=self.__errorLog)

        self.__location = "{}/XC".format(location)
        self.__XC_FUNCTIONAL = XC_FUNCTIONAL.XC_FUNCTIONAL(
            errorLog=self.__errorLog,
            changeLog=self.__changeLog,
            location=self.__location,
        )
        self.__VDW_POTENTIAL = VDW_POTENTIAL.VDW_POTENTIAL(
            errorLog=self.__errorLog,
            changeLog=self.__changeLog,
            location=self.__location,
        )
        self.__XC_GRID = XC_GRID.XC_GRID(
            errorLog=self.__errorLog,
            changeLog=self.__changeLog,
            location=self.__location,
        )
        ...
        self.__changeLog.append(
            {
                "Date": datetime.datetime.now(),
                "Module": "DFT.XC",
                "Variable": "DENSITY_CUTOFF",
                "Success": True,
                "Previous": self.__DENSITY_CUTOFF,
                "New": val,
                "ErrorMessage": None,
                "Location": self.__location,
            }
        ...
        self.__changeLog.append(
            {
                "Date": datetime.datetime.now(),
                "Module": "DFT.XC",
                "Variable": "DENSITY_SMOOTH_CUTOFF_RANGE",
                "Success": True,
                "Previous": self.__DENSITY_SMOOTH_CUTOFF_RANGE,
                "New": val,
                "ErrorMessage": None,
                "Location": self.__location,
            }
            ...
            self.__changeLog.append(
                {
                    ...
                    "Variable": "FUNCTIONAL_ROUTINE",
                    "Success": True,
                    "Previous": self.__FUNCTIONAL_ROUTINE,
                    "New": val,
                    "ErrorMessage": None,
                    "Location": self.__location,
                }
        ...
        self.__changeLog.append(
            {
                "Date": datetime.datetime.now(),
                "Module": "DFT.XC",
                "Variable": "GRADIENT_CUTOFF",
                "Success": True,
                "Previous": self.__GRADIENT_CUTOFF,
                "New": val,
                "ErrorMessage": None,
                "Location": self.__location,
            }
